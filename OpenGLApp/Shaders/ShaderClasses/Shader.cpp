...
namespace AppEngine
{
	...
	void Shader::SetUniform4f(const std::string& name, glm::vec4& vec, unsigned int program)
	{
		GLCall(glUniform4fv(GetUniformLocation(name, program), 1, &vec[0]));
	}
	...
	std::string Shader::ParseShader(const std::string& source)
	{
		...
		std::cout << "is_open: " << stream.is_open() << "\n";
		std::cout << "good:    " << stream.good() << "\n";
		std::cout << "fail:    " << stream.fail() << "\n";
		std::cout << "bad:     " << stream.bad() << "\n";
		std::cout << std::filesystem::current_path() << '\n';
		std::cout << std::filesystem::absolute(source) << '\n';
		if (!stream.is_open())
		{
			std::cerr << "Failed to open shader: " << source << std::endl;
			return "";
		}
		...
		while (getline(stream, line))
		{
			if (line.find("#include") != std::string::npos)
			{
				...
				std::string filePath = line.substr(firstPos + 1, lastPos - firstPos - 1);
				size_t slashPos = source.find_last_of('/\\');
				std::string sourceCatalog = source.substr(0, slashPos + 1);
				std::string additionalData = ParseShader(sourceCatalog + filePath);
				ss << additionalData;
			}
			if (!isExtensionLine)
				ss << line << '\n';
