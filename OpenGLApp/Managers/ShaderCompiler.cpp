...
#define PROGRAM_NAME "DefaultShaderProgram"
	...
	void ShaderCompiler::AddShadersToProgram(std::vector<Shader*> Shaders)
	{
		...
		GLint success;
		glGetProgramiv(m_Program.at(activeProgram), GL_LINK_STATUS, &success);

		if (!success)
		{
			char infoLog[512];
			glGetProgramInfoLog(m_Program.at(activeProgram), 512, nullptr, infoLog);
	...
	{
		if (!glIsProgram(m_Program.at(activeProgram))) {
			std::cerr << "Error: Program ID " << m_Program.at(activeProgram) << " is not a valid OpenGL program." << std::endl;
		}

		// 2. SprawdŸ status zlinkowania
		GLint linkStatus;
		glGetProgramiv(m_Program.at(activeProgram), GL_LINK_STATUS, &linkStatus);
		if (linkStatus == GL_FALSE) {
			std::cerr << "Error: Program ID " << m_Program.at(activeProgram) << " failed to link." << std::endl;
		}
